...
const submitForm = () => {
    form.post('/products', {
        onSuccess: (data) => {
            notify('Producto registrado con éxito.');
            router.push('/products');
        },
        onError: (errs) => {
            notify('Error al registrar el producto. Verifique los datos.', 'error');
        }
    });
};
</script>
                    ...
                    <form @submit.prevent="submitForm">
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div>
                                    <label for="price" class="block text-sm font-semibold text-gray-700">Precio (Bs.)</label>
                                    <input
                                        v-model="form.price"
                                        id="price"
                                        type="number"
                                ...
                                <div>
                                    <label for="stock" class="block text-sm font-semibold text-gray-700">Inventario (Stock)</label>
                                    <input
                                        v-model="form.stock"
                                        id="stock"
                                        type="number"
